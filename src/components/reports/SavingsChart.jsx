export default function SavingsChart({ months }) {
  ...
  return (
    <div
      className='flex h-[180px] w-full items-center justify-between animate-rise-in'
      aria-label='최근 6개월 절약액'
    >
        ...
        return (
          <div key={month.key} className='flex h-[180px] w-7 flex-col items-center gap-[6px]'>
            <div className='relative h-[152px] w-full overflow-hidden rounded-t-[20px] bg-report-chart-track'>
              <div
                className='absolute inset-x-0 bottom-0 min-h-5 origin-bottom rounded-t-[20px] bg-report-chart-gradient animate-chart-bar-rise'
                style={{ height, animationDelay: `${120 + index * 70}ms` }}
                title={`${month.label} ${month.amount.toLocaleString()}원`}
              />
            </div>
            <span className='h-[22px] w-full text-center text-body2 text-gray-500'>
              {month.label}
            </span>
          </div>
        )
