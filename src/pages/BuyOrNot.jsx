...
function ConcernCard({ record, onClick }) {
  ...
  return (
    <Card
      className={`bg-white p-6 flex flex-col gap-3 cursor-pointer active:opacity-80 ${expired ? 'border-2 border-error' : ''}`}
      onClick={onClick}
    >
...
export default function BuyOrNot() {
  ...
  const concerns = useMemo(() => loadHistory().filter((r) => r.choice === 'hold' && !r.checkin), [])
